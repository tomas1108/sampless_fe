    ...
    <div class="flex flex-col gap-10">
      ...
      >
        <div class="py-6 px-4 md:px-6 xl:px-7.5">
          <h4 class="text-xl font-bold text-black dark:text-white">Win/Lose History List</h4>
        </div>
        ...
        <div v-if="histories.length > 0">
          ...
          >
            ...
            <div class="col-span-2 flex items-center">
              <p class="text-sm font-medium text-black dark:text-white">
                {{ history?.match?.table?.name }}
              </p>
            </div>
            <div class="col-span-2 flex items-center">
              <p class="text-sm font-medium text-black dark:text-white">
                {{
                  history?.type ? (history?.type === 'win' ? '+' : '-') + '$' + history?.amount : ''
                }}
              </p>
            </div>
            <div class="col-span-2 flex items-center">
              <p
                ...
                :class="
                  history?.type === 'win' ? 'bg-success text-success' : 'bg-danger text-danger'
                "
              >
                {{ history?.type }}
              </p>
...
import webSocketService, { histories } from '@/websocket'
import { onBeforeUnmount, onMounted, ref } from 'vue'
import { formatDate } from '@/helper/format'
import { sortArrayByDate } from '@/helper/sort'

const pageTitle = ref('Win Or Lose History')
...
const fetchHistories = async () => {
  ...
  try {
    ...
    const response = await axiosInstance.get(`/histories/${route.query?.uid}?${queries.toString()}`)

    if (response.data.length < limit.value) {
      hasMore.value = false
    }
    histories.value.push(...response.data)
    sortArrayByDate(histories.value, 'createdAt')
    page.value++
  } catch (error) {
